...
from mcbo.utils.distance_metrics import (
    hamming_distance, 
    maxnorm_distance, 
...
class CasmopolitanTrManager(TrManagerBase):
    ...
    def __init__(self,
                 ...
                 min_perm_radius: Union[int, float],
                 max_perm_radius: Union[int, float],
                 init_perm_radius: Union[int, float],
                 numeric_distance: Callable = maxnorm_distance,
                 nominal_distance: Callable = hamming_distance,
                 perm_distance: Callable = kendall_tau_distance,
        ...
        self.is_mixed = self.is_numeric and (search_space.num_nominal + search_space.num_permutation) > 0
        ...
        if search_space.num_permutation > 0:
            self.register_radius('perm', min_perm_radius, max_perm_radius, init_perm_radius)
        ...
        self.register_distances(numeric_distance, nominal_distance, perm_distance)
        assert self.is_numeric or self.search_space.num_nominal > 0
        
    def register_distances(
        self,
        numeric_distance: Callable, 
        nominal_distance: Callable, 
        perm_distance: Callable,
    ) -> None:
        self.distances = {}
        self.distances['numeric'] = numeric_distance
        self.distances['nominal'] = nominal_distance
        self.distances['perm'] = perm_distance
    ...
    def adjust_tr_radii(self, y: torch.Tensor, **kwargs):
        ...
        if self.succ_count == self.succ_tol:  # Expand trust region
            ...
            if self.search_space.num_permutation_dims > 1:
                self.radii['perm'] = min(self.radii['perm'] * self.radius_multiplier, self.max_radii['perm'])
            if self.verbose:
            ...
            if self.search_space.num_permutation_dims > 1:
                self.radii['perm'] = self.radii['perm'] / self.radius_multiplier
            if self.verbose:
    ...
    def suggest_new_tr(self, n_init: int, observed_data_buffer: DataBuffer,
        ...
        inside_mask = filter_within_trust_region(
            x_observed, 
            tr_centre, 
            self.search_space, 
            self.radii, 
        ...
        self.data_buffer.append(x_observed[inside_mask], y_observed[inside_mask])
