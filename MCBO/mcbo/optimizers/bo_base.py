...
class BoBase(OptimizerBase):
    ...
    def get_point_sampler(self) -> Callable[[int], pd.DataFrame]:
        if self.init_sampling_strategy == "uniform":
            ...
            # for permutations - sample as usual and just check the order of the sobol elements -> transform to perm)
    ...
    def method_suggest(self, n_suggestions: int = 1) -> pd.DataFrame:
        ...
        if n_remaining:
            ...
            if len(self.constr_models) > 0:
                acq_evaluate_kwargs["constr_models"] = self.constr_models
                acq_evaluate_kwargs["out_upper_constr_vals"] = self.out_upper_constr_vals
