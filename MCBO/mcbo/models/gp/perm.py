...
class AugmentedSpearmanKernel(Kernel):
    ...
    def forward(self, x1: Tensor, x2: Tensor, diag: bool = False, last_dim_is_batch: bool = False, **params) -> Tensor:
        n_perm = x1.shape[-1]
        order_x1 = x1.argsort(dim=-1).unsqueeze(-2) / n_perm
        order_x2 = x2.argsort(dim=-1).unsqueeze(-3) / n_perm
        
        unnorm_dist = torch.pow((order_x1 - order_x2) / (self.lengthscale.unsqueeze(-2)), 2).sum(dim=-1)
        if diag:
